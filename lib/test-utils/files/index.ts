import { FileSystemItem, isDirectory, name } from "@lib/core";
...
export const clearDirectory = async (
  ...
  if (await exists(path)) {
    ...
    await Promise.all(
      files.map((filename) => removeFileMaybe(filename, path, keep))
    );
  }
};

const removeFileMaybe = async (
  filename: string,
  path: string,
  keep: string[]
): Promise<void> => {
  const shouldKeep = keep.some(
    (keepName) => join(path, filename) === join(path, keepName)
  );
...
export const tmpDirPath = (...path: string[]): string =>
  join(TEST_PROJECT_DIR, ...path);
